...
export default function Signup() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-start sm:justify-center bg-background px-4 md:px-6 py-10 sm:py-0 relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-64 h-64 md:w-96 md:h-96 bg-primary/10 rounded-full blur-3xl translate-x-1/2 -translate-y-1/2 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[300px] h-[300px] md:w-[500px] md:h-[500px] bg-blue-500/5 rounded-full blur-3xl -translate-x-1/4 translate-y-1/4 pointer-events-none" />
      ...
      >
        ...
        <Card className="rounded-[24px] sm:rounded-[40px] shadow-premium border-none p-1 sm:p-4 overflow-hidden bg-card/80 backdrop-blur-xl border border-white/20">
          <CardHeader className="text-center pt-8 px-6 pb-2">
            <CardTitle className="text-2xl sm:text-3xl font-black tracking-tight">Create Account</CardTitle>
            <CardDescription className="text-sm sm:text-base font-medium text-muted-foreground/60">Join thousands of smart savers today</CardDescription>
          </CardHeader>
          <CardContent className="p-6 sm:p-8">
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-1.5">
                      ...
                      <FormControl>
                        <Input placeholder="Alex Rivers" className="h-12 rounded-2xl bg-secondary/30 border-none px-4 font-bold focus-visible:ring-2 focus-visible:ring-primary/20" {...field} />
                      </FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-1.5">
                      ...
                      <FormControl>
                        <Input type="email" placeholder="you@example.com" className="h-12 rounded-2xl bg-secondary/30 border-none px-4 font-bold focus-visible:ring-2 focus-visible:ring-primary/20" {...field} />
                      </FormControl>
                      <FormMessage className="text-xs font-semibold text-destructive/90" />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-1.5">
                      <FormLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Secure Password</FormLabel>
                      <FormControl>
            ...
            <div className="relative my-10">
              ...
              <div className="relative flex justify-center"><span className="bg-card px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 border border-border/50">or start with</span></div>
            </div>
            ...
            <p className="text-center text-sm text-muted-foreground mt-8 font-medium">
              ...
              <Link to="/login" className="text-primary font-black hover:underline">Log in here</Link>
            </p>
          </CardContent>
        </Card>
      </motion.div>
